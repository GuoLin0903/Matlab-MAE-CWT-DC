...
curveOpt.shift_map_us      = [66 40 20];   % manual shift discussed earlier
...
% candidate features for simple, visual ranking
...
if basicOpt.enable && ~isempty(T)
    try
        ...
        if outOpt.save_diag_fig
            plot_basic_diagnostics(T, rankTab, selectedTab, outOpt.save_dir);
        end
    catch ME
...
save(fullfile(outOpt.save_dir, 'results.mat'), 'T', 'rankTab', 'selectedTab', 'extractNames', ...
    'outOpt','dataOpt','emdOpt','dispOpt','sepOpt','curveOpt','srcOpt','basicOpt');
...
function feat = extractfeati_struct(t_us, sig, fs, chSel, evSel)
    ...
    try
        ...
        % keep NaNs if ExtractFeati_OPT fails for a specific signal
    end
end
...
function plot_dual_view_AGU_curve(plotList, t_us, fs, dispOpt, figTitle, ref_max_raw, raw_ylim, curve_data, curveOpt)
    ...
    for i = 1:nRows
        ...
        if dispOpt.force_same_rawrow && isfinite(ref_max_raw) && ref_max_raw > 0
            norm_base = ref_max_raw;   % shared color scale across ALL rows
        else
            norm_base = all_max;
        end
...
function add_curve_legend(ax, curveOpt)
    ...
    dy = 0.18*(yl(2)-yl(1));   % larger separation to avoid overlap
    hold on;
...
function plot_basic_diagnostics(T, rankTab, selectedTab, outDir)
    if ~exist(outDir, 'dir'), mkdir(outDir); end

    % 1) keep a very basic overview figure
    valid = isfinite(T.logRE) & isfinite(T.logRP);
    fig1 = figure('Color','w', 'Position', [60 60 900 680]);
    scatter(T.logRE(valid), T.logRP(valid), 70, T.distance_mm(valid), 'filled');
    hold on;
    lab = lower(string(T.source_label));
    idxSurf = valid & (lab=="surface");
    idxEdge = valid & (lab=="edge");
    plot(T.logRE(idxSurf), T.logRP(idxSurf), 'ko', 'MarkerSize', 9, 'LineWidth', 1.1);
    plot(T.logRE(idxEdge), T.logRP(idxEdge), 'k^', 'MarkerSize', 9, 'LineWidth', 1.1);
    grid on;
    ...
    if isempty(selectedTab), return; end

    nSel = height(selectedTab);
    fig2 = figure('Color','w', 'Position', [60 60 1300 max(300, 230*nSel)]);
    tl = tiledlayout(nSel, 2, 'TileSpacing','compact', 'Padding','compact');
    lab = lower(string(T.source_label));
    d = double(T.distance_mm);
    cmap = parula(256);
    dmin = min(d(isfinite(d))); dmax = max(d(isfinite(d)));
    if ~isfinite(dmin) || ~isfinite(dmax) || dmin == dmax
        dmin = 0; dmax = 1;
    end

    for i = 1:nSel
        fname = selectedTab.feature{i};
        x = double(T.(fname));

        % left: by source type
        nexttile;
        hold on;
        idxSurf = isfinite(x) & (lab=="surface");
        idxEdge = isfinite(x) & (lab=="edge");
        xs = 1 + 0.12*(rand(nnz(idxSurf),1)-0.5);
        xe = 2 + 0.12*(rand(nnz(idxEdge),1)-0.5);
        scatter(xs, x(idxSurf), 46, d(idxSurf), 'filled', 'MarkerEdgeColor','k');
        scatter(xe, x(idxEdge), 46, d(idxEdge), 'filled', '^', 'MarkerEdgeColor','k');
        xlim([0.5 2.5]); xticks([1 2]); xticklabels({'surface','edge'});
        grid on; ylabel(fname, 'Interpreter','none');
        title(sprintf('%s | by source', fname), 'Interpreter','none');

        % right: by distance
        nexttile;
        hold on;
        if any(idxSurf)
            scatter(d(idxSurf), x(idxSurf), 46, d(idxSurf), 'filled', 'MarkerEdgeColor','k');
        end
        if any(idxEdge)
            scatter(d(idxEdge), x(idxEdge), 52, d(idxEdge), 'filled', '^', 'MarkerEdgeColor','k');
        end
        grid on; xlabel('Distance [mm]'); ylabel(fname, 'Interpreter','none');
        title(sprintf('%s | vs distance', fname), 'Interpreter','none');
    end
    colormap(tl, cmap);
    cb = colorbar; cb.Layout.Tile = 'east'; cb.Label.String = 'Distance [mm]';
    title(tl, 'Basic feature view: large source difference and small distance drift', 'FontWeight','bold');
    saveas(fig2, fullfile(outDir, 'Diag_basic_top_features.png'));
    close(fig2);
end
