...
curveOpt.dist_mm  = 20;      % 距离 [mm]
curveOpt.shift_us = 95;       % 曲线整体平移 [us]（对齐触发/起点偏移）
...
dispOpt.disable_local_norm = false; % false: 保留“弱行局部归一化”（更易看弱信号）
                                   % true : 强制所有行用全局归一化（更适合严格对比幅值）

% -------- (E) A0 / S0 自动分组（基于 CWT 低频能量占比）--------
sepOpt.split_freq_Hz  = 150e3; % <= split 认为低频
sepOpt.ratio_th       = 0.55;  % lowRatio >= ratio_th -> A0-like
...
eemdOpt.noise_std_ratio = 0.2;     % 0.1~0.3 常用
...
saveOpt.save_mat           = true;   % 保存结果 .mat（同目录）
...
[resStruct_emd] = group_and_recon(raw_sig, imfs_emd, res_emd, fs, sepOpt, dispOpt);
results.EMD = resStruct_emd;

% ---- 4.2 EEMD ----
fprintf('\n=== Running EEMD  ===\n');
[imfs_eemd, res_eemd, info_eemd] = eemd_decompose(raw_sig, eemdOpt, emdInnerOpt);
[resStruct_eemd] = group_and_recon(raw_sig, imfs_eemd, res_eemd, fs, sepOpt, dispOpt);
...
[resStruct_ce] = group_and_recon(raw_sig, imfs_ce, res_ce, fs, sepOpt, dispOpt);
...
function R = group_and_recon(raw_sig, imfs, res, fs, sepOpt, dispOpt)
